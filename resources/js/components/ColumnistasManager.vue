        ...
        <div class="mb-4 flex gap-4 items-center">
            ...
            <div class="flex-1">
                <input
                    ...
                    @input="handleSearch"
                    type="text"
                    placeholder="Buscar columnistas por nombre o email..."
...
export default {
    ...
    methods: {
        ...
        async checkEditParam() {
            ...
            if (editId) {
                try {
                    const response = await axios.get(`/api/columnistas/${editId}`);
                    this.openEditModal(response.data);
                    // Limpiar el parámetro de la URL
                    window.history.replaceState({}, '', '/columnistas-vue');
                } catch (error) {
                    console.error('Error al cargar columnista para editar:', error);
                }
            }
        },
        ...
        handleSearch() {
            ...
            this.searchTimeout = setTimeout(() => {
                this.fetchColumnistas(1); // Buscar desde la página 1
            }, 500);
        }
    }
};
